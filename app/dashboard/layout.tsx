...
import { LayoutDashboard, Search, Briefcase, MessageSquare, Sparkles, Building2, Users, Inbox } from "lucide-react";
...
export default async function DashboardLayout({
  ...
  const role = (clerkUser.publicMetadata.role as string) || "candidate";
    ...
    <div className="flex h-screen w-full bg-zinc-950 text-zinc-100">
      <aside className="hidden md:flex w-64 flex-col border-r border-zinc-800/80 bg-zinc-900/30 backdrop-blur-xl">
        ...
        <nav className="flex-1 space-y-1 p-4 text-sm font-medium">
          {navItems.map((item) => (
            <a key={item.name} href={item.href} className="flex items-center gap-3 rounded-lg px-3 py-2 text-zinc-400 transition-all hover:bg-zinc-800/50 hover:text-zinc-100">
        ...
        <div className="p-4 border-t border-zinc-800/80">
          <div className={`rounded-xl p-4 border ${role === 'recruiter' ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-indigo-500/10 border-indigo-500/20'}`}>
            <p className={`text-xs font-semibold ${role === 'recruiter' ? 'text-emerald-300' : 'text-indigo-300'}`}>{role === 'recruiter' ? 'B2B Mode' : 'Pro Tip'}</p>
            <p className="mt-1 text-xs text-zinc-400">
              {role === 'recruiter' ? "You are viewing real talent from our database." : "Use AI Search to find jobs that match your resume."}
            </p>
          </div>
        </div>
      </aside>
